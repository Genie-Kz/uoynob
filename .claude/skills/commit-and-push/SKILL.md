...
- **自分が行った変更だけ**をステージする。`git add -A` で、無関係な既存の未追跡ファイル（`.gitignore` 整備前に紛れ込んだファイル等）や別作業の変更を巻き込まない。
- ユーザーが「ついでに他の変更も一緒に」と明示したときだけ、それらも含める。
- 1コミット1関心。関心が分かれるなら `git add -p` 等で分割する。
