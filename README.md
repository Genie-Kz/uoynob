...
```
src/
├─ types/          型定義（Monster, Skill, Attribute, Ability, Pickup, Stats ...）
├─ constants/      定数（耐性・系統サイズ分類・ビルド計算ルール・ステータス計算ルール・ナビ構成・ピックアップ分類）
...
├─ api/            データ取得層（静的JSONの読み込み＋Promiseキャッシュ）
├─ composables/    Vue の状態ロジック（useMonsters / useSkills / useBuildSimulator / useTheme ...）
├─ presentation/   表示用ビューモデル生成（耐性セル）
├─ components/     再利用 UI（ResistanceGrid, MonsterTable, MonsterIcon, StatsBar, FamilyTreeIv, PickerModal ...）
├─ views/          画面（Home / MonsterList / MonsterDetail / SiteSearch / Search / Skill... / Pickup / Simulator...）
├─ router/         ルート定義（ハッシュ方式）
└─ test/           テスト用フィクスチャ
