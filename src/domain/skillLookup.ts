 ...
 */
export function createMonsterIdResolver(monsters: Monster[]): (skillMonsterId: string) => string | null {
  ...
  return (skillMonsterId: string) =>
    fullIdSet.has(skillMonsterId) ? skillMonsterId : (firstVariantIdByNo.get(skillMonsterId) ?? null);
}
